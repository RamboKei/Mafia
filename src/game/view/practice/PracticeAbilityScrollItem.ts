 ...
 */
class PracticeAbilityScrollItem extends ScrollListItem
{
	protected _nodeContainer:BaseDisplayObjectContainer;
	private _expProgress:ProgressBar = null;
	private _conditionTxt:BaseTextField;
	private _attrTxt:BaseTextField;
	private _attrAddTxt:BaseTextField;
	private _curCfg:Config.PracticeItemCfg;
	private _isBtnEnable:boolean = false;
	private _goTrick:boolean = false;
	private _detailVTxt:BaseTextField;
	...
	{	
		this._curCfg = data
		let servantId = this._curCfg.servantId;
		let wifeId = this._curCfg.wifeId;
		...
		{
			iconPath = "wife_half_"+this._curCfg.wifeId;
		}
		let iconImg = BaseLoadBitmap.create(iconPath );
        iconImg.width = 100;
        iconImg.height = 100;
		...
		let attrTxt = ComponentManager.getTextField("",20,TextFieldConst.COLOR_BROWN);
		attrTxt.x = this._conditionTxt.x;
		attrTxt.y = this._conditionTxt.y +27;
		this.addChild(attrTxt);
		this._attrTxt = attrTxt;

		let attrAddTxt = ComponentManager.getTextField("",20,TextFieldConst.COLOR_BROWN);
		...
		let goBtn = ComponentManager.getButton(ButtonConst.BTN_SMALL_RED,"practice_goBtnTxt",this.goBtnHandler,this);
	...
	protected refreshUI()
	{
		...
		let ptaskVo:PracticeTaskVo = Api.practiceVoApi.getPracticeTaskInfo(this._curCfg.id);
		let taskSinfo = Api.practiceVoApi.getPracticeTaskAccumulation(ptaskVo);
		let condition = this._curCfg.conditionList[ptaskVo.stage];
		let conditionV = taskSinfo.conditionV;
		let attrType = this._curCfg.type;
		if(taskSinfo.isComp)
		{	
			this._conditionTxt.text = LanguageManager.getlocal("practice_compTxt");
			this._attrAddTxt.text = "";
			let curStr = LanguageManager.getlocal("servantInfo_attrTxt"+attrType+"_1")+ "+" + String(taskSinfo.curV);
			...
			{
				let compFlag = BaseBitmap.create("practice_comp_flag");
			...
			if(this._curCfg.servantId){
				 ...
				 tarname = LanguageManager.getlocal("wifeName_"+this._curCfg.wifeId);
			}
			if(ptaskVo.stage == 1){
				if(this._curCfg.servantId){
			...
			let addStr = LanguageManager.getlocal("servantInfo_attrTxt"+attrType+"_1")+ "+" + String(condition.effect);
	...
	protected unlockCallBackHandler(event:egret.Event)
    {
		...
		let addValue = rdata.data.addAttrValue;
        let str = LanguageManager.getlocal("servantInfo_attrTxt" + this._curCfg.type+"_1") + "+" + addValue;
	...
	protected showWifeOrServantInfo()
	{
