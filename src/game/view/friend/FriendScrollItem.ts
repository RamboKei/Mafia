...
class FriendScrollItem extends ScrollListItem
{
	...
	protected initItem(index:number,data:any):void
	{
		...
		if(this._uiType == FriendScrollItem.UITYPE1 || this._uiType == FriendScrollItem.UITYPE7){
			...
			if(textStr == LanguageManager.getlocal("chat_time4")){
				...
				onLineTxt.textColor = TextFieldConst.COLOR_WARN_GREEN2;
			}
	...
	protected initUIType2()
	{
		if(Api.friendVoApi.isAppliedByUid(this._uiData.uid)){
			...
			applyBtn.x = this._bg.x + this._bg.width - applyBtn.width - 15;
	...
	protected initUIType3()
	{
		...
		acceptBtn.x = this._bg.x + this._bg.width - acceptBtn.width - 15;
		acceptBtn.y = this._bg.y + 65;
		acceptBtn.name = "acceptBtn";
		this.addChild(acceptBtn);

		let ignoreBtn = ComponentManager.getButton(ButtonConst.BTN_SMALL_RED,"friendsBtnTxt16",this.ignoreBtnHandler,this);
		ignoreBtn.x = acceptBtn.x  - ignoreBtn.width - 10;
	...
	protected initUIType4()
	{
		...
		shieldCancelBtn.x = this._bg.x + this._bg.width - shieldCancelBtn.width - 15;
	...
	{
		let applyCancelBtn = ComponentManager.getButton(ButtonConst.BTN_SMALL_RED,"friendsBtnTxt14",this.applyCancelBtnHandler,this);
		applyCancelBtn.x = this._bg.x + this._bg.width - applyCancelBtn.width - 15;
		applyCancelBtn.y = this._bg.y + 68;
