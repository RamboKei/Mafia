...
class FriendItem3 extends BaseDisplayObjectContainer
{
	...
	public init(bottomH:number):void
	{
		...
		let denyBtn = ComponentManager.getButton(ButtonConst.BTN_SMALL_RED,"friendsBtnTxt6",this.denyBtnHandler,this);
		denyBtn.x = buttombg.x + buttombg.width - denyBtn.width*2 - 50;
		denyBtn.y = buttombg.y + buttombg.height/2 - denyBtn.height/2;
		denyBtn.name = "denyBtn";
		this.addChild(denyBtn);

		let  batchBtn= ComponentManager.getButton(ButtonConst.BTN_SMALL_YELLOW,"friendsBtnTxt21",this.batchBtnHandler ,this);
		batchBtn.x = buttombg.x + buttombg.width - batchBtn.width - 30;
		batchBtn.y = buttombg.y + buttombg.height/2 - batchBtn.height/2;
	...
	protected doRefreshList()
	{
		...
		if(dataList.length == 0){
			...
			if(!tipTxt){
				tipTxt = ComponentManager.getTextField(LanguageManager.getlocal("friends_emptyTip5"),20,TextFieldConst.COLOR_LIGHT_YELLOW);
