...
class InviteViewTab4ScrollItem  extends ScrollListItem
{
    ...
    protected initItem(index:number,userPid:string)
    {
		...
		head.x = 170 - head.width*head.scaleX;
        ...
        // 区
        let serverTxt = ComponentManager.getTextField(Api.mergeServerVoApi.getAfterMergeSeverName(Math.floor(Api.inviteVoApi.getUserUidByPid(userPid)/1000000)),TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_BROWN)
        serverTxt.x = 368 - serverTxt.width/2;
        ...
        powerTxt.x = 520 - powerTxt.width/2;
