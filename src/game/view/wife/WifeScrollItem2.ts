...
class WifeScrollItem2 extends ScrollListItem 
{
	...
	protected initItem(index:number,wifeItemCfg:Config.WifeItemCfg):void
	{
		this.width = 640;
		...
		let icon:BaseBitmap = BaseLoadBitmap.create(wifeItemCfg.body);
		icon.x = 60;
		//todo icon的宽高尺寸需要固定
		icon.y = 0;
		this.addChild(icon);
		...
		let nameTF = ComponentManager.getTextField(wifeItemCfg.name,TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_WHITE);
		...
		let descTF = ComponentManager.getTextField(wifeItemCfg.desc,TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_LIGHT_YELLOW);
		descTF.width = 255;
