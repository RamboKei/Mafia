...
class DecreePapeInfoView  extends BaseView
{
	...
	protected initView():void
	{
		...
		for (var index = 1; index <= paperKeys.length; index++) {
			...
			costTxt.text = "" + tmpcfg.gdCost;
			costTxt.y =	goldIcon.y+14;
			this._nodeRightContainer.addChild(costTxt);
			if(isDissTime)
			{
				goldIcon.x = paperbg.x + 180;
				costTxt.text = LanguageManager.getlocal("decreePaperDetail_costTxt",[""+tmpcfg.gdCost,String(tmpcfg.gdCost -leveeGemDiscount*tmpcfg.gdCost) ]);
			}
		...
		tipTxt.text = LanguageManager.getlocal("decreePaperTipTxt",[""+leveeTime[0],""+leveeTime[1]]);
	...
	protected PaperImgClickHandler(obj:any,param:number)
	{
		...
		let mesObj = {
			...
			msg: LanguageManager.getlocal("decreeChooseTxt1",[""+costGold,gdName]),
			id : 1,
			useNum : costGold,
			linespacing : 6,
			height : 250
