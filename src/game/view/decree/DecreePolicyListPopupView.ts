...
class DecreePolicyListPopupView  extends PopupView
{
	...
	protected initView():void
	{
		...
		for (var index = 0; index < keys.length; index++) {
			...
			descTxt.text = LanguageManager.getlocal("decreePolicy_Desc"+decCfg.id,[""+decCfg.addTimes,""+addaddExtent]);
			descTxt.x = nameTxt.x;
			descTxt.y = nameTxt.y + nameTxt.height + 10;
			scrolNode.addChild(descTxt);

			startY += policybg.height + 4;
		}

		let rect = new egret.Rectangle(0,0,this.viewBg.width,484);
	...
	protected ProlicyImgClickHandler(obj:any,param:string)
	{
		...
		let mesObj = {
			...
			msg: LanguageManager.getlocal("decreePolicyChooseTxt1",[""+costNUm]),
			id : 1,
			useNum : costNUm,
			linespacing : 6,
			height : 250
