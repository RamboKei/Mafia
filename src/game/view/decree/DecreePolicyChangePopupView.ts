...
class DecreePolicyChangePopupView extends PopupView
{
	...
	protected initView():void
	{
		...
		for (var index = 0; index <= 1; index++) {
			...
			descTxt.text = LanguageManager.getlocal("decreePolicy_Desc"+policyCfg.id,[""+policyCfg.addTimes,addaddExtent]);
			// descTxt.text = LanguageManager.getlocal("decreePolicy_Desc"+tmpSpid);
			descTxt.x = nameTxt.x;
