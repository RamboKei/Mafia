...
class DailyTaskRewardPreviewPopuiView  extends PopupView
{
	...
	{	
        ...
        let rewardX = rbg.x + (data.type == AcMayDayView.AID||data.type == AcConst.AID_ACARCHER ? ((500 - 108 * rewardArr.length - 10 * (rewardArr.length - 1)) / 2): 10);//rewardX = rbg.x + (500 - 108 * rewardArr.length - 10 * (rewardArr.length - 1)) / 2;
        ...
        for (var index = 0; index < rewardArr.length; index++) {
            ...
            if(numLb && data.type != AcMayDayView.AID && data.type != AcConst.AID_ACARCHER)
            {
                numLb.visible = false;
            }
            if (index > 0 )
            {   
                ...
                if( index%4 == 0){
                    rewardX = rewardX = rbg.x + (data.type == AcMayDayView.AID||data.type == AcConst.AID_ACARCHER ? ((500 - 108 * rewardArr.length - 10 * (rewardArr.length - 1)) / 2): 10);
