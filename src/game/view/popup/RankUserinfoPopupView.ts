...
class RankUserinfoPopupView  extends PopupView
{
	...
	{	
		...
		let temX = this.viewBg.x+this.viewBg.width - 230;
		let temY = fanImg.y;
		...
		userContainer = Api.playerVoApi.getPlayerPortrait(curLv,UIData.pic);
		...
		if(UIData.promoteType){
			...
			if(api.isKing()){
				...
				xrenBtn.x = (this.viewBg.width - xrenBtn.width) / 2;
			...
			this.setLayoutPosition(LayoutConst.righttop, DesctText, bg0, [30,50]);
			...
			priBtn.x = dinnerBtn.x - 10 - priBtn.width;
			...
			this._friendsBtn.x = this._shieldBtn.x + this._shieldBtn.width + 10;
