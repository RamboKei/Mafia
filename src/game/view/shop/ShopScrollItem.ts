	...
	protected initItem(index:number,data:any)
    {
		...
		else
		{
			...
			for(let i = 0;i < contentList.length;i++)
			{
				...
				let itemNameTF:BaseTextField = ComponentManager.getTextField(rewardItemVo.name,TextFieldConst.FONTSIZE_TITLE_SMALL,rewardItemVo.nameColor);
				itemNameTF.x = itemNameBg.x + itemNameBg.width/2 - itemNameTF.width/2;
