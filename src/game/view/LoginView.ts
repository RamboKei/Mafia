...
class LoginView extends BaseView
{
	...
	private getCfg(successCallback:Function,callbackThisObj:any,isRetry?:boolean):void
	{
		...
		NetManager.http.get(ServerCfg.svrCfgUrl,reqData,(newdata:any)=>{
			...
			if (newdata&&newdata.idcardinfo && newdata.idcardinfo.switch === 1) {
				...
				// 简易模式还是正常模式
				if (newdata.idcardinfo.idcardnormal) {
					GameData.idcardNormal = newdata.idcardinfo.idcardnormal;
				}
				// 是否进入过正常模式
				if (newdata.idcardinfo.enternormal) {
					GameData.idcardEnterNormal = newdata.idcardinfo.enternormal;
				}
				// 几类用户
				if (newdata.idcardinfo.idcardusertype) {
					GameData.idcardType = newdata.idcardinfo.idcardusertype;
				}
				// 是否去调收费查询接口的三态开关
				if (newdata.idcardinfo.idcardnofreeapiswitch) {
					GameData.idcardNoFreeApiSwitch = newdata.idcardinfo.idcardnofreeapiswitch;
				}
				// 是否是以前认证过的用户
				if (newdata.idcardinfo.info && newdata.idcardinfo.info.idnumber) {
					GameData.idcardType = 3;
					Api.realnameVoApi.setIdcardInfo(newdata.idcardinfo.info.idnumber, null, 1, (err)=>{
						if (err) {
							App.CommonUtil.showTip(LanguageManager.getlocal("realname_neterror"));
						}
					});
				}
				if (GameData.idcardType === 0) {
					// 如果是0类用户，就弹提示
	...
	private showLogoAndLoginBtn():void
	{
		...
		if (App.DeviceUtil.isWyw()) {
			let versionText:BaseTextField=ComponentManager.getTextField("版本v1.2.30.2",14,TextFieldConst.COLOR_WHITE);
	...
	private initTestTouch():void
	{
		...
		leftMk.graphics.drawRect(0,0,60,100);
			...
			if(this._touchNum==10)
			{
				if(App.DeviceUtil.IsHtml5())
				{
					...
					if(pathName.indexOf("gt_test")<0)
					{
						...
						newurl=curl.replace(pathName,testpathName);
					}
					if(newurl!=curl)
					{
						this.createPanel(()=>{
	...
	private createPanel(callback:(...args)=>void,btnStr?:string):void
	{
		...
		var confirmBtn:BaseButton = ComponentManager.getButton("loginres_btn3","",()=>{
			if(callback&&usernameInput.text=="gttest2018")
			{
				callback();
				panel.dispose();
				maskSp.dispose();
	...
	{	
		...
		if (PlatformManager.checkIsHeYue()) 
		{	
