...
class AdultSearchView extends PopupView
{
	...
	protected initView():void
	{
		...
		//查询按钮
		let queryBtn:BaseButton = ComponentManager.getButton(ButtonConst.BTN_BIG_YELLOW,"find",this.queryBtnHandler,this);
		...
		view.setLayoutPosition(LayoutConst.horizontalCenterbottom, killBtn, view.viewBg, [0,80]);
	...
	private useCallback(data:any):void
	{
		...
		else if(data.data.data.data)
		{
			...
			if( Math.floor(data.data.data.data.ruid / 1000000) != Number(ServerCfg.selectServer.zid)){
				var str:string =LanguageManager.getlocal("atkraceErrordes3");
				App.CommonUtil.showTip(str);
				view.refreshView()
				return;
