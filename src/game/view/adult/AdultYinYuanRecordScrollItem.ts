...
class AdultYinYuanRecordScrollItem extends ScrollListItem
{
	...
	public initItem(index:number,data:any):void
	{
		...
		let qinjiaTxt = ComponentManager.getTextField(LanguageManager.getlocal(`adultyinyuanrecordTitle${data.index + 1}`), 22, TextFieldConst.COLOR_WHITE);
		...
		let empTxt = ComponentManager.getTextField(LanguageManager.getlocal(`acPunishNoData`), 22, TextFieldConst.COLOR_WHITE); 
		...
		if(data.show){
			...
			arr.sort((a,b)=>{
				if(data.param == 'notsadun'){
					...
					let baifanga = Api.adultVoApi.getAdultInfoVoById(a.childid).visit == a.uid;
					let baifangb = Api.adultVoApi.getAdultInfoVoById(b.childid).visit == b.uid;
