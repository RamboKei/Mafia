...
class EmperorWarVsView extends CommonView {
    ...
    private createLine():void{
        ...
        for(let i in vs){
            ...
            for(let j in vs[i]){
                ...
                let line_1 = BaseLoadBitmap.create(`empline1`);
                line_1.name = `line${numb}-1`;
                line_1.width = 90;
                line_1.scaleX = Number(j) == 1 ? -1 : 1;
                line_1.scaleY = Number(i) < 3 ? 1 : -1;
                group.setLayoutPosition(Number(i) < 3 ? LayoutConst.lefttop : LayoutConst.leftbottom, line_1, manbg, [(manbg.width)/2 + (Number(j) == 0 ? -7 : 5),manbg.height - 5]);
                group.addChild(line_1);
            }
            //2阶段战斗线 4条2按钮
            let line_2 = BaseLoadBitmap.create(`empline1`);
            line_2.name = `line${i}-2`;
            line_2.visible = false;
            line_2.width = 170;
            line_2.scaleX = Number(i) % 2 == 0 ? -1 : 1;
            line_2.scaleY = Number(i) < 3 ? 1 : -1;
            group.setLayoutPosition(LayoutConst.lefttop, line_2, group, [line_2.scaleX == 1 ? 150 : (163 - group.x),line_2.scaleY == 1 ? (200 - group.y) : (-32 - group.y)]);
            group.addChild(line_2);
            //3阶段战斗线 2条1按钮
            if(Number(i) % 2){
                let line_3 = BaseLoadBitmap.create(`emptline1`);
                ...
                group.setLayoutPosition(LayoutConst.lefttop, line_3, group, [line_2.x + line_2.width - 14, Number(i) < 3 ? (line_2.y + 32 + 15 - group.y) : ( line_2.y - 32 - 15 - group.y)]);
            ...
            for(let i in arr){
                ...
                if(line){
                    ...
                    if(App.CommonUtil.check_dragon()){
                        ...
                        button = BaseLoadBitmap.create('empfight');
                    ...
                    if(round == '3'){ //line.height/2 + 10
                        group.setLayoutPosition(LayoutConst.lefttop, fu1, line, [34,(view._midgroup.height)/2 - group.y * 2 + fu1.height / 2]);
                    }
                    else{
    ...
    private hfangclick(evt, btnIndex:string):void{
        ...
        if(App.CommonUtil.check_dragon()){
            ...
            flag = evt.currentTarget.resourceName == 'emphfang';
        }
        if(flag){
            switch(btnIndex){
                case '1_1':
    ...
    private reDrawLine(period){
        ...
        for(let i in vs){
            ...
            for(let j in vs[i]){
                ...
                if(line){
                    line.setload(period < 3 ? `empline${linetype}` : `emptline${linetype}`);
                }
            }
            let line3 : any; 
            if(period == 1){
    ...
    private createMid():void{
        ...
        for(let i in vs){
            ...
            for(let j in numArr){
                ...
                let bgtype = 1;
                let group = view[`_vsgroup${i}`];
                let manbg : BaseLoadBitmap = null;
                if(data){
                    if(data.uid == Api.playerVoApi.getPlayerID()){
                        bgtype = 2;
                    }
                    manbg = BaseLoadBitmap.create(`empvsmanbg${bgtype}`);
                    ...
                    manbg = BaseLoadBitmap.create(`empvsmanbg1`);
                    manbg.name = `manbg${numb}`;
                    manbg.width = 154;
                    manbg.height = 172;
                    group.setLayoutPosition((Number(j) == 0 ? LayoutConst.lefttop : LayoutConst.righttop), manbg, group, [0,0], true);
