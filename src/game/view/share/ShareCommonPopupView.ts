...
class ShareCommonPopupView extends PopupView
{
	...
	private _wxshare:[string,string,string,string] = null;
	...
	{	
		...
		// PlatformManager.share(this.shareSuccessCallback,this);
		//是微信平台
		if(App.DeviceUtil.isWXgame()){
			...
			let shareTitle = "";
			let shareContent = this._wxshare[0] + "," + this._wxshare[1];
			...
			RSDKHelper.guideShare(content,this.shareSuccessCallback, shareId);
		} else{
			//分享失败
