...
{	
	...
	protected refreshKingBtn(isAfterUp?:boolean)
	{
		...
		if(pid >= 20){
			if(isAfterUp ){
				...
				if(Api.prestigeVoApi.isBallLightUpEnable())
				{
	...
	protected makeDomstages(isStage1:boolean = false)
	{
		...
		if(pid < maxNum || isStage1){
			...
			let numStr = LanguageManager.getlocal("baseNum"+baseNum);
	...
	protected refreshDomAfterLight()
	{
		...
		if(lastcfg.canEmperor == 1){
			...
			egret.Tween.get(openDg,{loop:false}).wait(1000).to({x:tarX,y:tarY,scaleX:0.5,scaleY:0.5},600).call(()=>{
				...
				this.removeChildFromContainer(openDg);
...

		{
			...
			egret.Tween.get(dgbone,{loop:false}).wait(1000).call(()=>{
				...
				this._preDomNode.removeChild(dgbone);
				if(dgbone2)
				{	
					this._preDomNode.removeChild(dgbone2);
					this._preDomNode.removeChild(lastMidimg);
				}
				this.refreshNextDomImg();
			},this);
		}
	...
	protected getResourceList(): string[] {
		return super.getResourceList().concat([
			...
			"prestige_status3","prestige_status4","prestige_status5","prestige_status6","prestige_titlebg","wifestatus_namebg",
			"prestige_prerogative1_down","prestige_prerogative2_down","prestige_prerogative3_down","prestige_displaybg",
		]);
	}
