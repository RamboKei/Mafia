...
class AcDoubleSeventhAVGView extends CommonView
{
	...
	{	
        ...
        let guidePic:string[] = [];
        let allAvg = view.vo.getAvgConfig(view.param.data.buidId);
	...
	{	
		...
		this._curConfig = view.vo.getAvgConfig(view.param.data.buidId)[this._curIdx];//RookieCfg.getRookieCfg(this._curIdx);
		this.showPage();
	}
	...
	private showPage():void
	{
		...
		if (this._curConfig.descId) {
			...
			if (this._curConfig.clickContinue) {
				...
				this._codonLength = 0;
				this._descText.text="";
				this._descContent = LanguageManager.getlocal("DoubleSevenAVGDialog"+this._curConfig.descId);
				needShoot = true;
			}
			else {
				this._descText.text = LanguageManager.getlocal("DoubleSevenAVGDialog"+this._curConfig.descId);
