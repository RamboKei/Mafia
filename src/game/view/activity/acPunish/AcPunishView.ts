...
class AcPunishView extends AcCommonView {
	...
	private showText(): void {
		...
		let bigBg: BaseLoadBitmap = BaseLoadBitmap.create(bgres,rect);
		...
		if (this.code == "1"){
			...
			let rect:egret.Rectangle=egret.Rectangle.create();
			rect.setTo(0,0,220,612);
			let bodyPic: BaseLoadBitmap = BaseLoadBitmap.create("punish_body2",rect)
			...
			let rect:egret.Rectangle=egret.Rectangle.create();
			rect.setTo(0,0,220,612);
			let bodyPic: BaseLoadBitmap = BaseLoadBitmap.create("punish_body",rect)
			this._bodyContainer.addChild(bodyPic);;
			bodyPic.y = 0;
		...
		if(this.code == "3")
		{
			...
			let btnImg = "btn_buy_tool";
			if(this.code == "6")
			{
				 btnImg = "btn_buy_tool_6"
			}
			let buyBtn = ComponentManager.getButton(btnImg,"",this.butItemBtnCilck,this);
			buyBtn.x = 0;
			buyBtn.y = GameConfig.stageHeigth - this.container.y - bottomBg.height - 120;
			this.addChildToContainer(buyBtn);
		}
	...
	//点灯动画
	private doCodeSixLightAni()
	{
		let aniStr = "punish6_"
		let itemClip = ComponentManager.getCustomMovieClip(aniStr,5,100);
		// itemClip.x = 0;
		// itemClip.y = 470;
		...
		itemClip.setEndCallBack(()=>{
			...
			this._bodyContainer.removeChild(itemClip);
			itemClip = null;
			this._touchSwitch = true;
		},this);
		let key = (this._selectIndex+1).toString();
	...
	private sendNetMessage(key):void{
		let view = this;
	...
	private nextAni2(m:any)
	{
		...
		// this._headPic.setRes("punish_head2_2");
		...
		egret.Tween.get(m).wait(200).to({alpha:0},500).call(function(m:BaseDisplayObjectContainer){
            if(m)
            {
				...
				// this._headPic.setRes("punish_head2_1");
                m.dispose();
				// this.nextAni2(m);
				this._touchSwitch = true;
				this._wordsContanier.visible = false;
