...
class AcPunishBuyItemPopupView extends PopupView
{
	...
	{		
		...
		if(Api.switchVoApi.checkPunishVip()){
			let itemName:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("acPunishBuyItemDesc"),TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_BLACK);
			itemName.setPosition(this.viewBg.width - itemName.width - 20,icon1Bg.y + icon1Bg.height/2 - addGoldBtn.height/2 + 6);
