...
desc : 端午活动 划龙舟
*/
class AcDragonBoatDayViewTab1 extends CommonViewTab
{
	...
	{	
		...
		awardbg.width = 510;
		awardbg.height = 136;
		view.setLayoutPosition(LayoutConst.horizontalCenterbottom, awardbg, view, [0,118]);
		view.addChild(awardbg);
		view._awardbg = awardbg; 
		if(AcDragonBoatDayView.CODE=="2")
		{
		...
		if(AcDragonBoatDayView.CODE=="1")
		{
			...
			let boatclip = ComponentManager.getCustomMovieClip("dragonboat",2,800);
			boatclip.width = 504;
			boatclip.height = 148;
		...
		let rankBtn = ComponentManager.getButton('dragonboatrank', '', view.rankClick,view);
		...
		let zziImg = BaseBitmap.create('dragonboatitem'+AcDragonBoatDayView.CODE); 
		zziImg.setScale(0.5); 
		...
		view.setLayoutPosition(LayoutConst.horizontalCentertop, progress, bg, [0,dbboatbg.height+40]);
		progress.setDragIcon('dragonboathead'+AcDragonBoatDayView.CODE);
	...
	private fresh_jindu(jindu):void{
		...
		let str = totalMeter >= view.vo.getEndMeter() ? LanguageManager.getlocal(`DragonBoatDayEnd_`+AcDragonBoatDayView.CODE) : (LanguageManager.getlocal('DragonBoatDayTotalRiceNum_'+AcDragonBoatDayView.CODE, [view.vo.getTotalRiceNum().toString()]));
		...
		view._curRiceTxt.text = LanguageManager.getlocal('DragonBoatDayRiceNum_'+AcDragonBoatDayView.CODE, [curRice]);
		...
		if(nextData){
			...
			view._curRiceTxt.text = LanguageManager.getlocal('DragonBoatDayRiceNum_'+AcDragonBoatDayView.CODE, [prevData.needMeter.toString()]);
			view.setLayoutPosition(LayoutConst.horizontalCenterverticalCenter, view._curRiceTxt, view._dbnumricecurbg, [0,2]);
			nextRice = curData.needMeter.toString();
		}
		view._nextRiceTxt.text = LanguageManager.getlocal('DragonBoatDayRiceNum_'+AcDragonBoatDayView.CODE, [nextRice]);
	...
	private fresh_rewward(cur_jindu){
		...
		view._prevAwardRiceTxt.text = LanguageManager.getlocal('DragonBoatDayRiceNum_'+AcDragonBoatDayView.CODE,[prevRice]);
		...
		view._curAwardRiceTxt.text = LanguageManager.getlocal('DragonBoatDayRiceNumAward_'+AcDragonBoatDayView.CODE,[curMeter.toString()]);
		...
		view._nextAwardRiceTxt.text = LanguageManager.getlocal('DragonBoatDayRiceNum_'+AcDragonBoatDayView.CODE,[nextMeter.toString()]);
		view.setLayoutPosition(LayoutConst.rightverticalCenter, view._nextAwardRiceTxt, view._nextBtn, [-20,0]);
		
		view._scrollList.refreshData(view.vo.gerCurRiceAward(view._curJindu));
