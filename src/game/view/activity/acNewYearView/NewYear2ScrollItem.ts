...
class NewYear2ScrollItem  extends ScrollListItem
{
	...
	{	 
	 	let tmpVo = <AcNewYearVo>Api.acVoApi.getActivityVoByAidAndCode(AcNewYearView.AID,AcNewYearView.CODE); 
		let tmpCfg = <Config.AcCfg.NewYearCfg> Config.AcCfg.getCfgByActivityIdAndCode(AcNewYearView.AID, AcNewYearView.CODE);
		if(tmpVo.isStart==false)
		{
			App.CommonUtil.showTip(LanguageManager.getlocal("acPunishEnd"));
			return;
		...
		else
		{
			...
			let itemName:string = LanguageManager.getlocal("acNewYearPackageName"+ NewYear2ScrollItem.TADAY+"_"+AcNewYearView.CODE)
