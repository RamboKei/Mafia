...
class AcVipShopViewScrollItem  extends ScrollListItem
{
    ...
    {    
        ...
        else
        {
            ...
            let str2 = LanguageManager.getlocal("vipshopbuydes",[currNextVipLevel+"",data.buyNum[currNextVipLevel]+""]);
        ...
        //购买过的价格 
        let acVo = <AcVipShopVo>Api.acVoApi.getActivityVoByAidAndCode(AcVipShopView.AID,AcVipShopView.CODE);
        this._acVo=acVo;
        var buyNum =0;
        if(acVo.shop)
        {   
	    ...
	    if(buyNum){
           ...
           this._buyNum = data.buyNum[currVipLevel]-buyNum;
            //当前的
             let str = LanguageManager.getlocal("vipshopbuydes",[currVipLevel+"",data.buyNum[currVipLevel]-buyNum+""]);
             activityTimerText.text=str;
             if(data.buyNum[currVipLevel]-buyNum==0)
             {
                let str = LanguageManager.getlocal("vipshopbuydes_red",[currVipLevel+"",data.buyNum[currVipLevel]-buyNum+""]);
                 activityTimerText.text=str;
             }

             if(currVipLevel!=Config.VipCfg.getMaxLevel())
             {
                let str2 = LanguageManager.getlocal("vipshopbuydes",[currNextVipLevel+"",data.buyNum[currNextVipLevel]-buyNum+""]);
                var num =this.getNextVipNum(data.buyNum[currVipLevel],data.buyNum[currNextVipLevel]);
                let maxNum =this._databuyNum.length-1;
                if(num&&num>0&&num!=maxNum)
                {
