	...
	public initItem(index:number,data:any):void
	{
		...
		switch(Number(this._itemData.questType))
		{
			...
			{	
				...
				{
					strTop = LanguageManager.getlocal("AcMazeViewTaskRandom2",[this._itemData.value]);
				}
		...
		let scheduleStr = LanguageManager.getlocal("AcMazeViewTaskPlan",[scheduleNum < this._itemData.value?"<font color=0xce1515>" + scheduleNum +"</font>":"<font color=0x21eb39>" + scheduleNum +"</font>",this._itemData.value])
