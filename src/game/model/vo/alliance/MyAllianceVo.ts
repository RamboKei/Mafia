...
class MyAllianceVo extends BaseVo
{
	...
	public initData(data:any):void
	{
		if(data)
		{
			...
			if(data.info != null)
			{
				let isreset:boolean = false; //是否重置了帮会建设门客
				if(this.info && this.info.taskservant &&this.info.taskservant != data.info.taskservant )
				{
					let taskservant = data.info.taskservant;
					for (var key in this.info.taskservant) {
						if (!taskservant.hasOwnProperty(key)) {
							isreset = true;
							break;
						}
					}
				...
				this.info = data.info;
				if(isreset){
					App.MessageHelper.dispatchEvent(MessageConst.MESSAGE_ALLIANCE_TASK_RESET_SERVANT);
				}
			}
			if(data.apply != null)
			{
