...
class AdultVoApi extends BaseVoApi
{
	...
	{
		if((this._marryList.length && this._marryList.length > 0 || (this.getVisitNum() > 0) || this.getReceiveData().length > 0 || this.isSadunCanVisit())){
	...
	public getAdultIsInMarry(id):boolean
	{
		...
		let lastTime = 0;
		if(adultInfoVo.pro && adultInfoVo.pro[0] && adultInfoVo.pro[1] < 3){
	...
	public isSadunCanVisit(uid?){
		if(Api.switchVoApi.checkopenSadun()){
			if(this._sadunVo.info && Api.adultVoApi.getAdultVoList().length > 0){
				for(let i in this._sadunVo.info){
					if(this._sadunVo.info[i].times >= Config.SadunCfg.needNum && this._sadunVo.info[i].visiting == '' && (uid ? this.isChildCanVisit(uid) : true)){
						return true;
					}
				}
			}
