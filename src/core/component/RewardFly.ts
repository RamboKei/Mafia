...
class RewardFly extends BaseDisplayObjectContainer
{
	...
	public init(icon:string,message:string,itemtype:number=0,subNode:BaseDisplayObjectContainer,isHalve?:boolean):void
	{	
		...
		let msgTF:BaseBitmapText|BaseTextField = ComponentManager.getBitmapText(message,TextFieldConst.FONTNAME_ITEMTIP);
		if(iconBt)
		{
