...
var AcVipShopViewScrollItem = (function (_super) {
    ...
    AcVipShopViewScrollItem.prototype.initItem = function (index, data) {
        ...
        if (currVipLevel == 0 && data.buyNum[currVipLevel] == 0) {
            ...
            if (data.buyNum[currVipLevel] == 0) {
                ...
                str = LanguageManager.getlocal("vipshopbuydes", [currVipLevel + "", data.buyNum[currVipLevel] + ""]);
            }
        ...
        if (currVipLevel == Config.VipCfg.getMaxLevel()) {
            ...
            var str2 = LanguageManager.getlocal("vipshopbuydes", [currNextVipLevel + "", data.buyNum[currNextVipLevel] + ""]);
            ...
            if (num && num > 0 && num != maxNum) {
                str2 = LanguageManager.getlocal("vipshopbuydes", [num + "", data.buyNum[num] + ""]);
                nextActivityTimerText.text = str2;
            }
        }
        //购买过的价格 
        var acVo = Api.acVoApi.getActivityVoByAidAndCode(AcVipShopView.AID, AcVipShopView.CODE);
        this._acVo = acVo;
        var buyNum = 0;
        if (acVo.shop) {
        ...
        if (buyNum) {
            ...
            //当前的
            var str_1 = LanguageManager.getlocal("vipshopbuydes", [currVipLevel + "", data.buyNum[currVipLevel] - buyNum + ""]);
            activityTimerText.text = str_1;
            if (data.buyNum[currVipLevel] - buyNum == 0) {
                var str_2 = LanguageManager.getlocal("vipshopbuydes_red", [currVipLevel + "", data.buyNum[currVipLevel] - buyNum + ""]);
                activityTimerText.text = str_2;
            }
            if (currVipLevel != Config.VipCfg.getMaxLevel()) {
                var str2 = LanguageManager.getlocal("vipshopbuydes", [currNextVipLevel + "", data.buyNum[currNextVipLevel] - buyNum + ""]);
                var num = this.getNextVipNum(data.buyNum[currVipLevel], data.buyNum[currNextVipLevel]);
                var maxNum = this._databuyNum.length - 1;
                if (num && num > 0 && num != maxNum) {
                    str2 = LanguageManager.getlocal("vipshopbuydes", [num + "", data.buyNum[num] - buyNum + ""]);
                }
                nextActivityTimerText.text = str2;
            }
        }
        ...
        this._buyBtn.x = icon.x - 20;
        this._buyBtn.y = bg.y + bg.height - this._buyBtn.height - 10;
    ...
    AcVipShopViewScrollItem.prototype.buyHandler = function (param) {
        ...
        NetManager.request(NetRequestConst.ACTIVITY_BUYVIPSHOP, { "activeId": AcVipShopView.AID + "-" + AcVipShopView.CODE, shopId: this._data.id });
    };
