...
var ChallengeAutoRewardsPopupView = (function (_super) {
    ...
    ChallengeAutoRewardsPopupView.prototype.receiveData = function (data) {
        ...
        for (var i = 0; i < allKey.length; i++) {
            ...
            fightNum++;
            var smallId = (cid % 41) % 8;
            //结算
            if (smallId == 0 || (i + 1) == allKey.length) {
                var bcid = Math.floor(cid / 41) + 1;
                var mcid = Math.floor((cid - 1) % 41 / 8) + 1;
    ...
    ChallengeAutoRewardsPopupView.prototype.initView = function () {
        ...
        rect.setTo(30, grayBg.y + 10, this.viewBg.width, grayBg.height - 20);
        ...
        var finalText = ComponentManager.getTextField(LanguageManager.getlocal(finialKey), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_WARN_YELLOW);
    ...
    ChallengeAutoRewardsPopupView.prototype.showContainerAnim = function () {
        if (this._containerTab.length > this._curShowIdx) {
            ...
            this._curShowIdx++;
            tempContainer.x = -550;
            egret.Tween.get(tempContainer).wait(500).to({ x: -30 }, 300);
    ...
    ChallengeAutoRewardsPopupView.prototype.getRewardInfoContainer = function (info) {
        ...
        var rewardBg = BaseBitmap.create("public_9_bg14");
        rewardBg.width = 504;
        rewardBg.height = 190;
        ...
        var targetText = ComponentManager.getTextField(LanguageManager.getlocal("challengeRewardTitle", [String(info.bigId), targetStr, String(info.midId)]), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_WARN_YELLOW2);
        targetText.setPosition(rewardBg.x + 20, 14);
        bgContainer.addChild(targetText);
        var lineImg2 = BaseLoadBitmap.create("rank_line");
        lineImg2.width = 500;
        lineImg2.height = 2;
        lineImg2.x = this.viewBg.width / 2 - lineImg2.width / 2;
        lineImg2.y = targetText.y + targetText.height + 7;
        bgContainer.addChild(lineImg2);
        var fightText = ComponentManager.getTextField(LanguageManager.getlocal("challengeRewardFight", [String(info.fightCont), String(info.curSd)]), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_BLACK);
        ...
        var awardText = ComponentManager.getTextField(LanguageManager.getlocal("challengeReward"), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_BLACK);
