...
class TabBarGroup extends BaseDisplayObjectContainer
{
	 ...
	 * @param param          参数
	 */
	public init(buttonName:string|string[],textArr:Array<string>,callback:({param:any,index:number})=>void,handler:any,param?:any):void
	{
		...
		if(textArr.length > 0)
		{
			...
			for(var i = 0;i < textArr.length;i++)
			{
				...
				tb.init(btnName,textArr[i],this.clickButtonHandler,this,null,TextFieldConst.FONTSIZE_TITLE_COMMON);
				this.addChild(tb);
				this._tbArr.push(tb);
			}
			this.buildTabTar();
	...
	public addRedPoint(index:number):void
	{
		...
		for(var i = 0;i < l;i++)
		{
			if(i == index)
			{
				App.CommonUtil.addRedDotToBDOC(this._tbArr[i]);
			}
		}
	}
	...
	public removeRedPoint(index:number):void
	{
		...
		for(var i = 0;i < l;i++)
		{
			if(i == index)
			{
				App.CommonUtil.removeRedDotFromBDOC(this._tbArr[i]);
			}
		}
	}
	 ...
	 * @param icon 
	 */
	public showStatusIcon(index:number,icon:string = ""):void
	{
		for(var i = 0;i < this._tbArr.length;i++)
		{
			if(i == index)
