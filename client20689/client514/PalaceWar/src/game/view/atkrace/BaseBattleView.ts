...
class BaseBattleView extends CommonView
{
	...
	private _topMaxValue:number = 0;
	private _topCurValue:number = 0;
	private _topProgressType:number = 1; // 1 ,分数  2 百分比
	/**
	 * 底部血条
	 */
	protected _bottomProgress:ProgressBar=null;
	private _bottomMaxValue:number = 0;
	private _bottomCurValue:number = 0;
	/**
	 * 上面的人物
	 */
	...
	private _damageText:BaseBitmapText = null;
	private _beAttackClip:CustomMovieClip = null;
	private _heroMask : BaseShape = null;
	...
	protected setTopProgress(value:number,maxValue?:number, progressWidth?:number , type:number=1):void
	{
		...
		if (this._topProgressType == 1) {
			...
			this._topProgress.setText(this._topCurValue.toString() +"/"+ this._topMaxValue);
		}
		else {
	...
	protected setBottomProgress(value:number,maxValue?:number):void
	{	
		if (value < 0) {
			value=0;
		}
		if (this._bottomProgress == null) {
			this._bottomProgress = ComponentManager.getProgressBar("progress8","progress7_bg",GameConfig.stageWidth);
		...
		this._bottomProgress.setText(this._bottomCurValue.toString()+"/"+ this._bottomMaxValue);
		this._bottomProgress.setPercentage( this._bottomCurValue / this._bottomMaxValue );
	}

	protected setUpHero(picName:string,info?:any,type?:number):void
	{
		if (this._upHero) {
		...
		this._upHero.init(picName, info, type, 0);
	...
	protected setDownHero(picName:string,info?:any,type?:number):void
	{
		if (this._downHero) {
		...
		this._downHero.init(picName, info ,type, 1);
	...
	protected showBeAttackAnim():void
	{
		...
		this._beAttackClip.setPosition(this._heroArray[1].x + this._heroArray[1].width/2 - tempBitmap.width/2 ,beAttackHero.y + beAttackHero.height/2  - tempBitmap.height/2);
		...
		this._damageText = ComponentManager.getBitmapText("-"+Math.floor(this._damage).toString(),fontName);
		this._damageText.setPosition(this._heroArray[1].x + this._heroArray[1].width/2 - this._damageText.width/2 ,beAttackHero.y + beAttackHero.height/2  - 50);
