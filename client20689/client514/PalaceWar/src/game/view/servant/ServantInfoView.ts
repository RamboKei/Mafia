...
class ServantInfoView  extends CommonView
{
	...
	public initView():void
	{
		...
		let servant_infobg = BaseBitmap.create("servant_infobg");
		servant_infobg.y = -20;
		...
		for (var index = 0; index < specialAbility.length; index++) {
			...
			//图 
			let specialityIcon = BaseBitmap.create("servant_speciality"+element)
	...
	protected refreshInfoAfterUpdate(p:any)
	{
		...
		let nameTxt = this._nodeContainer.getChildByName("nameTxt");
        this._nodeContainer.addChild( nameTxt);
