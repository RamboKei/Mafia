...
class DinnerFindPopupView extends PopupView
{
	...
	{	
		...
		let inputTF = ComponentManager.getInputTextField(TextFieldConst.COLOR_WHITE,TextFieldConst.FONTSIZE_TITLE_SMALL,466,44,"public_9_bg5",LanguageManager.getlocal("inputPlayerId"),0xa4917f);
		...
		confirmBtn.setPosition(this.viewBg.width - confirmBtn.width - 55, inputTF.y+inputTF.height+12);
	...
	private showFindInfo():void
	{
		...
		this._dinnerInfoContainer.setPosition(50, 215);
		this.addChildToContainer(this._dinnerInfoContainer);

		let dinnerHost:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("dinnerHost"),TextFieldConst.FONTSIZE_CONTENT_COMMON);
		this._dinnerInfoContainer.addChild(dinnerHost);

		let dinnerType:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("dinnerType"),TextFieldConst.FONTSIZE_CONTENT_COMMON);
		dinnerType.y = 33;
		this._dinnerInfoContainer.addChild(dinnerType);

		let dinnerSeat:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("dinnerSeat")+":",TextFieldConst.FONTSIZE_CONTENT_COMMON);
		dinnerSeat.y = 66;
		this._dinnerInfoContainer.addChild(dinnerSeat);

		let dinnerCountDown:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("dinnerCountDown"),TextFieldConst.FONTSIZE_CONTENT_COMMON);
