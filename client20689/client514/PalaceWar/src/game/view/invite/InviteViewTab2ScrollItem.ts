 ...
 */
class InviteViewTab2ScrollItem extends ScrollListItem
{
	//领取按钮
	...
	protected initItem(index:number,data:{cfgId:string, arriveCount:number})
    {
		...
		this.data = data;
		// 配置信息
		let cfgInfo = Config.InvitefriendCfg.friendPower[data.cfgId];
		...
		// 权势达到
		let countLabel:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("inviteFriendPower",[String(cfgInfo.needPower)]),TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_BROWN);
	...
	private doGetRewardCallback(event:egret.Event):void
	{
		...
		if (ret && this.senderIsMe) {
			...
			let cfgInfo = Config.InvitefriendCfg.friendPower[this.data.cfgId]
