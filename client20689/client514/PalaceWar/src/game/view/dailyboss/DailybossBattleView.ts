class DailybossBattleView extends BaseBattleView
{
	...
	private showDefenedWinResult():void
	{
		if(!this._defenedWinContainer)
		{
			...
			let killerTxt:BaseTextField=ComponentManager.getTextField(LanguageManager.getlocal("dailybossTitle1"),TextFieldConst.FONTSIZE_BUTTON_COMMON,TextFieldConst.COLOR_LIGHT_YELLOW);
			killerTxt.lineSpacing=5;
			killerTxt.textAlign = egret.HorizontalAlign.CENTER;
			killerTxt.setPosition(killBg.x+(killBg.width-killerTxt.width)/2,killBg.y+160);
			this._defenedWinContainer.addChild(killerTxt);

			let personNumTxt:BaseTextField=ComponentManager.getTextField(LanguageManager.getlocal("dailybossDefendWin"),TextFieldConst.FONTSIZE_BUTTON_COMMON);
			personNumTxt.setPosition(killBg.x+(killBg.width-personNumTxt.width)/2,killerTxt.y+killerTxt.height+20);
