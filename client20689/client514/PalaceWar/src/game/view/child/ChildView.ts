...
class ChildView extends CommonView {
	...
	public initView(): void {
		...
		if (Api.childVoApi.getChildNum() > 0) {
			...
			{	
				...
				let addstr = LanguageManager.getlocal("wifeStatusProAdd2",[String((wifestatusVo.star*starEffect).toFixed(1))]);
	...
	{
		let childIndex = Api.childVoApi.getChildIndexVoById(childId);
	...
	private refreshItem(){
		...
		let index = Api.childVoApi.getChildIndexVoById(this._childInfoVo.id);
		let childScrollItem = <ChildScrollItem>this._scrollList.getItemByIndex(index);
