...
class AcMayDayViewTab1 extends CommonViewTab
{
	...
	{ 
        ...
        circleGroup.y = 20;
        //转盘
        let turnTable = BaseLoadBitmap.create('acturntable_bg');
        turnTable.x = 30;
        turnTable.y = -180;
        ...
        circleGroup = null;
        //排行榜按钮
        let rankBtn = ComponentManager.getButton('acturntable_rankicon', '', view.rankClick, this);
        rankBtn.x = 10
        rankBtn.y = -180;
        ...
        for(let i = 0; i < total; ++ i){
            ...
            //画直线
            let line = BaseLoadBitmap.create('acturntable_line');
            line.width = 327 / 2;   
        ...
        //添加指针
        let point = BaseLoadBitmap.create('acturntable_point');
        ...
        for(let i in buy_arr){
            ...
            //购买文字
            let buytext = ComponentManager.getTextField(LanguageManager.getlocal('acPunishBuyItemBuy'), 24, TextFieldConst.COLOR_BLACK);
            ...
            //添加购买次数
            let buynumtext = ComponentManager.getTextField(`x${num}`, 24,TextFieldConst.COLOR_BLACK);
            ...
            //推荐图标
            if(num == 10){
                let tuiJian = BaseLoadBitmap.create('acturntable_tjbg');
        ...
        //当前次数
        let numBg = BaseLoadBitmap.create("acmaydaynumbg");
        numBg.y = -190;
		...
		for (let index = 0; index < rewardList.length; index++) {
			...
			let lightImg =  BaseLoadBitmap.create("acturantable_taskbox_light");
