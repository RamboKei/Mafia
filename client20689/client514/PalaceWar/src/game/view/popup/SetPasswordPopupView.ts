...
class SetPasswordPopupView extends PopupView
{
	...
	{	
		...
		NetLoading.show();
		NetManager.http.get(this._urlPre,{t:"3kolduserget",ct:"import",ac:"getinfo",userid:this._userId,gameid:this._gameId,service_id:GameData.curZoneID},(data:any)=>{
			NetLoading.hide();
			if(data && data.data)
			{	
	...
	{	
		...
		NetLoading.show();
		NetManager.http.get(this._urlPre,{t:"3koldusersetpwd",ct:"import",ac:"ChangePassword",userid:this._userId,gameid:this._gameId,password:text1,service_id:GameData.curZoneID},(data:any)=>{
			NetLoading.hide();
			if(data && data.code == 0)
			{
				ViewController.getInstance().openView(ViewConst.POPUP.DOWNLOADPACKAGEPOPUPVIEW);
